"""Render lane 4: every fixture, two arms, two seeds, sequentially, one server.

Same receipts discipline as lanes 1-3: live ``/object_info`` preflight for
every class and model filename (INCLUDING both encoder loaders, since the two
arms load different classes), purity re-proved on the graphs actually queued,
outputs read back from ``/history``, temporal stats per leg. Best effort per
the no-VRAM-ceremony ruling -- the fp8 encoder is ~2.6 GB larger than the GGUF
one, so an OOM on the official arm is a plain recorded fault, not a project.
...
THE CONTRAST IS A NODE CLASS SWAP, so the submit gate declares it explicitly
via ``--expect-class``. Every OTHER class change, and any wiring change at all,
still aborts the lane.
...
SEEDS = [42, 20260821]
ARMS = ["ours", "official"]
OUT_PREFIX = "baseline_output/lane4_wan_text_encoder"
...
def gate_submissions(fixture, seed, submitted, receipts, manifest):
    ...
    declared = [
        'clip.clip_name="umt5_xxl_fp8_e4m3fn_scaled.safetensors"',
        'save.filename_prefix="%s"' % leg_prefix(fixture, "official", seed),
    ]
    argv = [str(paths["ours"]), str(paths["official"])]
    for item in declared:
        argv += ["--expect", item]
    argv += ["--expect-class", "clip=CLIPLoader"]
