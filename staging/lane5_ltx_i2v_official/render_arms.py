"""Render lane 5: every fixture, two arms, two seeds, sequentially, one server.

THE I2V PATH. Unlike lane 3 these graphs carry a conditioning still
(``LoadImage`` -> ``LTXVImgToVideoConditionOnly``), which is what production
runs by default. The still is a preflight-checked model input for that reason.

THE CONTRAST IS A BUNDLE: the official LoRA file AND its 0.5 strength move
together, because that pair is the official configuration. Both are declared to
the submit gate; a win cannot be attributed to either alone.
...
SEEDS = [42, 20260821]
ARMS = ["ours", "official"]
OUT_PREFIX = "baseline_output/lane5_ltx_i2v_official"
...
MODEL_INPUTS = {
    ...
    "VAELoader": ["vae_name"],
    # i2v conditions on a still, so the image is a MODEL-CLASS input here: a
    # missing conditioning still must fail at preflight, not at submit.
    "LoadImage": ["image"],
}
