"""Render lane 7: every fixture, two arms, two seeds, sequentially, one server.

Same receipts discipline as lane 1: live ``/object_info`` preflight for every
class and model filename (including the CPU-pinned encoder the ADAPT depends
on), purity re-proved on the graphs actually queued, outputs read back from
``/history``, temporal stats per leg. Best effort per the no-VRAM-ceremony
ruling -- an OOM is a recorded fault, not a forensic project.

The seed node here is ``RandomNoise.noise_seed`` (not KSampler.seed as in lane
1), and the terminal is ``decode`` -- a ``SaveImage`` is appended identically
to both arms so frames are judged at native 1664x960 with no codec in the path.
...
SEEDS = [42, 20260821]
ARMS = ["ours", "soft"]
OUT_PREFIX = "baseline_output/lane7_ltx25_motion"
LEG_TIMEOUT_S = 2 * 60 * 60
MODEL_INPUTS = {
    "UnetLoaderGGUF": "unet_name",
    "CLIPLoaderGGUFCPU": "clip_name",
    "VAELoader": "vae_name",
...
def gate_submissions(fixture, seed, submitted, receipts, manifest):
    ...
    declared = [
        "i2v.strength=0.7",
        "refine_i2v.strength=0.7",
        'save.filename_prefix="%s"' % leg_prefix(fixture, "soft", seed),
    ]
    argv = [str(paths["ours"]), str(paths["soft"])]
    ...
    for fixture in wanted:
        ...
        for seed in SEEDS:
            for arm in ARMS:
                ...
                print("")
                print("[LEG] %s  sha256=%s  i2v=%s refine=%s seed=%s"
                      % (label, digest[:16],
                         graph["i2v"]["inputs"]["strength"],
                         graph["refine_i2v"]["inputs"]["strength"],
